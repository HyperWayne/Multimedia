...
if(strcmp(filterName,'low-pass')==1)
    ...
    for n = ceil(-N/2):floor(N/2)
        if( n == 0)
            ...
            outputFilter(n+middle) = sin(wc*n)/(pi*n);
        end
...
if(strcmp(filterName,'bandpass')==1)
    ...
    for n = ceil(-N/2):floor(N/2)
        if( n == 0)
          ...
          outputFilter(n+middle) = (sin(2*pi*fcutoff(2)*n)-sin(2*pi*fcutoff(1)*n)) /(pi*n);
        end
...
if (strcmp(windowName,'Hamming') == 1)
    for n = 1 : N
...
for n=1:length(inputSignal);
    for k = 1:N
        if ((n-k) < 1)
          ...
          outputSignal(n) = outputSignal(n) + inputSignal(n-k) * outputFilter(k);
        end
    end
end
